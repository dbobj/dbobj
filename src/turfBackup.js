...
function intersectionOfTwoFracTriangles (fracTriangle1, fracTriangle2) { // OK
	// return simple polygon or null
	if (isSubsetOf(fracTriangle1, fracTriangle2) {
		...
		for (fracLineSegment1 of fracLineSegments1) {
			...
			if (edgeOfIntersection) {
				edgesOfIntersection.push(edgeOfIntersection)
			}
		}
		if (edgesOfIntersection.length < 2) {
			return null
		}
		for (fracLineSegment2 of fracLineSegments2) {
			edgeOfIntersection = intersectionOfFracLineSegmentAndFracTriangle(fracLineSegment2, fracTriangle1)
			if (edgeOfIntersection && isNewFracEdge(edgeOfIntersection, edgesOfIntersection)) {
				edgesOfIntersection.push(edgeOfIntersection)
			}
		}
		result = edgesOfIntersection[0]
	...
	if (result && simplePolygonArea(result) < 0) {
		result.reverse()
	}
	return result
}
...
function partitionOfEdgesByUnionOfTwoFracTraiangles (fracTriangle1, fracTriangle2) {
	// return multipolygon
	if (isSubsetOf(fracTriangle1, fracTriangle2) {
		result = [ [ [ ...fracTriangle2, fracTriangle2[0] ] ] ]
		break
