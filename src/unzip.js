...
async function main () {
	...
	if (prevLots.deleted.length > 0) {
		await pool.query("DELETE FROM lot WHERE id IN ('" + prevLots.deleted.join("', '") + "')")
	}
	if (prevLots.expired.length > 0) {
		// await pool.query("DELETE FROM lot WHERE id IN ('" + prevLots.expired.join("', '") + "')")
	}

	for (feature of features) {
		if (feature.properties.LOTID == 1810021554) {
			newFeatures = [ feature ]
			break
		}
	}
	...
	/* newFeatures = features.filter(function (feature) {
		...
		return (prevLots.current.indexOf(feature.properties.LOTID.toString()) == -1 && prevLots.expired.indexOf(feature.properties.LOTID.toString()) == -1)
	}) */
	promises = []
	count = 1
	for (feature of newFeatures) {
		promise = new Promise(function (resolve) {
			...
			if (feature.geometry.type == "Polygon") {
				rawCoord = [ feature.geometry.coordinates ]
			} else {
				rawCoord = feature.geometry.coordinates ?? [ feature.geometry.rings ]
			}
			coord = truncateMultiPolygon(rawCoord)
			console.log(prevLots.expired.length + prevLots.current.length + count, lotID)
			count = count + 1
...
/*
function coordComplementOfMultiPolygon (theTurf) {
	// CALLS turfComplementOfPolygon, turfIntersectionOfManyPolygonsOrMultiPolygons
	coordPolygons = turf.getCoords(theTurf)
	turfComplements = coordPolygons.map(function (coordPolygon) {
		return turfComplementOfPolygon(turf.polygon(coordPolygon))
	})
	return turfIntersectionOfManyPolygonsOrMultiPolygons(...turfComplements)
}

function coordComplementOfPolygon (coord) {
	coordComplement = [[[[700000, 700000], [900000, 700000], [900000, 900000], [700000, 900000], [700000, 700000]], coord[0]]]
	for (i = 1; i < coord.length; i++) {
		coordComplement.push([ coord[i] ])
	}
	return turf.multiPolygon(coordComplement)
}

function coordDifference (turf1, turf2) {
	// CALLS turfComplementOfPolygon, turfComplementOfMultiPolygon
	if (turf2.geometry.type == "Polygon") {
		return turfIntersectionOfTwoPolygonsOrMultiPolygons(turf1, turfComplementOfPolygon(turf2))
	} else {
		return turfIntersectionOfTwoPolygonsOrMultiPolygons(turf1, turfComplementOfMultiPolygon(turf2))
	}
}

function coordIntersectionOfManyPolygonsOrMultiPolygons (...turfs) {
	// CALLS turfIntersectionOfTwoPolygonsOrMultiPolygons
	turfIntersection = turfs[0]
	for (i = 1; i < turfs.length; i++) {
		turfIntersection = turfIntersectionOfTwoPolygonsOrMultiPolygons(turfIntersection, turfs[i])
	}
	return turfIntersection
}

function coordIntersectionOfTwoPolygonsOrMultiPolygons (turf1, turf2) {
	if (turf1 && turf2) {
		turfIntersection = turf.intersect(turf1, turf2)
		return turf.truncate(turfIntersection, { precision: 3 })
	} else {
		return null
	}
}
...
function coordIntersectionOfTwoTriangles
...
function getBox (coord) {
	...
	return {
		...
		yMin: yMin,
		yMax: yMax,
		feature: turf.truncate(turf.multiPolygon(coord), { precision: 3 })
	}
}
...
function getBoxes (allPlansObj) {
	...
	for (plan in allPlansObj) {
		...
		boxesObj[plan] = getBox(truncateMultiPolygon(rawCoord))
	}
	return boxesObj
}
...
function getLotUses (lotCoord, files) {
	resultObj = {}
	unallocated = turf.truncate(turf.multiPolygon(lotCoord), { precision: 3 })
	for (file of files) {
		zone = JSON.parse(fs.readFileSync("../lotdb.github.io/land/zone/" + file + ".json").toString())
		if (zone.ZONE) {
			features = zone.ZONE.features
		} else {
		...
		for (feature of features) {
			...
			featureMultiPoly = turf.truncate(turf.multiPolygon(truncateMultiPolygon(rawCoord)), { precision: 3 })
			try {
			...
			if (intersection) {
				intersection = turf.truncate(intersection, { precision: 3 })
				if (intersection.geometry.type == "Polygon") {
					coord = truncateMultiPolygon([ turf.getCoords(intersection) ])
				} else {
					coord = truncateMultiPolygon(turf.getCoords(intersection))
				}
				...
				try {
					...
					unallocated = turfDifference(unallocated, intersection)
				}
				if (unallocated) {
					unallocated = turf.truncate(unallocated, { precision: 3 })
				} else {
					break
				}
			}
	...
	if (unallocated) {
		if (unallocated.geometry.type == "Polygon") {
			unallocatedCoord = truncateMultiPolygon([ turf.getCoords(unallocated) ])
		} else {
			unallocatedCoord = truncateMultiPolygon(turf.getCoords(unallocated))
		}
	...
	for (plan in boxes) {
		...
		if (compare(testBox, box)) {
			...
			if (intersection) {
				...
				if (intersection.geometry.type == "Polygon") {
					rawCoord = [ turf.getCoords(intersection) ]
				} else {
					rawCoord = turf.getCoords(intersection)
				}
				zones[plan] = multiPolygonArea(truncateMultiPolygon(rawCoord))
			}
		}
	}
...
async function insert (pool, table, arr) {
	...
	for (i = 0; i < arr.length; i++) {
		...
		if (i % 1000 == 999 || i + 1 == arr.length) {
			await pool.query("INSERT INTO " + table + " VALUES ?", [ temp ])
			temp = []
		}
	}
}
...
function multiPolygonArea (coord) {
    ...
    for (polygon of coord) {
    	for (simplePolygon of polygon) {
    		area = area + simplePolygonArea(simplePolygon)
    	}
    }
    return area
}
...
function truncateMultiPolygon (rawCoord) {
	result = rawCoord
	for (a of result) {
		for (b of a) {
			for (c of b) {
